...
def query_recorded_installed():
    apt_installed = set()
    if os.path.exists("/tmp/ubiquity/apt-installed"):
        with open("/tmp/ubiquity/apt-installed") as record_file:
...
def record_removed(pkgs, recursive=False):
    ...
    record_file = "/tmp/ubiquity/apt-removed"
...
def query_recorded_removed():
    ...
    if os.path.exists("/tmp/ubiquity/apt-removed"):
        with open("/tmp/ubiquity/apt-removed") as record_file:
            for line in record_file:
